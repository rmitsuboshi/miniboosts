...
/// // where `n_examples = data.shape().0` is 
...
/// let n_examples = data.shape().0 as f64;
/// 
...
impl<'a, F, L> GBM<'a, F, L>
{
    ...
    pub fn init_with_loss(sample: &'a Sample, loss: L) -> Self {
        ...
        let predictions = vec![0.0; n_examples];
...
impl<F, L> GBM<'_, F, L> {
    ...
    pub fn max_loop(&self) -> usize {
        ...
        (n_examples.ln() / self.tolerance.powi(2)) as usize
    }
          ...
          L: LossFunction,
{
    ...
    fn info(&self) -> Option<Vec<(&str, String)>> {
        let (n_examples, n_features) = self.sample.shape();
        let info = Vec::from([
    ...
    fn preprocess(&mut self) {
        ...
        self.predictions = vec![0.0; n_examples];
    }
        ...
        where W: WeakLearner<Hypothesis = F>,
    {
        ...
        let coef = {
            let y = self.sample.target();
            self.loss.best_coefficient(y, &predictions[..])
        };

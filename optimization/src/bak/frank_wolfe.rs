...
impl<F> FrankWolfe<F> {
    ...
    pub fn classic(&self, cur: Vec<f64>, nxt: Vec<f64>) -> Vec<f64> {
        let stepsize = 2f64 / (self.iteration as f64 + 2f64);
        interior_point(cur, nxt, stepsize)
    }
}
    ...
    where F: ObjectiveFunction,
{
    ...
    /// ```
    /// The last entry of `hypotheses` is `h_{t+1}` in the above update rule.
    pub fn short_step(&self, cur: Vec<f64>, nxt: Vec<f64>) -> Vec<f64> {
    ...
    ///     λ ∈ [0, 1]
    /// ```
    fn line_search(&self, cur: Vec<f64>, nxt: Vec<f64>) -> Vec<f64> {
        let dir = nxt.iter()
            .zip(&cur[..])
            .map(|(n, c)| n - c)
            .collect::<Vec<_>>();
    ...
    fn bpfw(&self, cur: Vec<f64>, nxt: Vec<f64>) -> Vec<f64> {
        let grad = self.objective.gradient(&cur[..]);
    ...
    pub fn next(&mut self, cur: Vec<f64>, nxt: Vec<f64>) -> Vec<f64> {
        let weights = match self.update_rule {
